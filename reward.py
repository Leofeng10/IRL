...
def parse_args() -> tuple[Args]:
    args = tyro.cli(Args)
    # accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps)
    # print("******************************************************************",accelerator.num_processes)
    args.world_size = 1
    print(args.local_micro_batch_size,args.gradient_accumulation_steps)
    ...
    time_tensor = torch.tensor(int(time.time()), device="mps")
...
def evaluate(args: Args, tokenizer, model, dataloader):
    model.eval()
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    for split in ["validation", "validation_cnndm"]:
        ...
        print("The number of samples in validation_dataset", len(validation_dataset))
    ...
    if args.run_eval:
        for eval_split in eval_dataloaders:
            evaluate_df = evaluate(args, tokenizer, model, eval_dataloaders[eval_split])
            for split, row in evaluate_df[["split", "accuracy"]].groupby(["split"]).mean().iterrows():
                ...
                print(f"eval/rm/{eval_split}/accuracy/split/{split}: {row['accuracy']}")
            ...
            print(f"eval/rm/{eval_split}/accuracy: {evaluate_df['accuracy'].mean()}")
    ...
    with torch.no_grad():
        for data in tqdm(norm_dataloader):
            ...
            unexpecte_reward_diff = unexpecte_reward_diff
            unexpecte_reward_diff_gt_rtol = unexpecte_reward_diff_gt_rtol
    ...
    if args.output_dir and args.num_train_epochs > 0:
        ...
        time_int = time_tensor[0].item()  # avoid different timestamps across processes
...
#     --run_eval \
#     --track \
#     --output_dir=models/EleutherAI/pythia-160m/reward_model_1 \
