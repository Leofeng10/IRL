...
if __name__ == "__main__":
    ...
    eval_datasets = {}
    eval_splits = ["validation", "validation_cnndm"]
    ...
    losses_list = []
    accuracies_list = []
    reward_preferred_list = []
    reward_rejected_list = []
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for data in dataloader:
            ...
            if (update % args.gradient_accumulation_steps) == 0:
                optimizer.step()
                optimizer.zero_grad()
                scheduler.step()
                ...
                reward_preferred_list.append(chosen_rewards.mean().item())
                reward_rejected_list.append(rejected_rewards.mean().item())

                train_accuracy = np.mean(accuracies_list[-args.gradient_accumulation_steps:])
                train_loss = np.mean(losses_list[-args.gradient_accumulation_steps:])
                train_chosen = np.mean(reward_preferred_list[-args.gradient_accumulation_steps:])
                train_rejected = np.mean(reward_rejected_list[-args.gradient_accumulation_steps:])

                writer.add_scalar("train/rm/loss", train_loss, global_step)
                writer.add_scalar("train/rm/accuracy", train_accuracy, global_step)
                writer.add_scalar("train/rm/chosen_rewards", train_chosen, global_step)
                writer.add_scalar("train/rm/rejected_rewards", train_rejected, global_step)
                writer.add_scalar("train/rm/lr", scheduler.get_last_lr()[0], global_step)
                print(f"{train_accuracy=}, {scheduler.get_last_lr()=}, {optimizer.param_groups[0]['lr']=}, {update=}")
